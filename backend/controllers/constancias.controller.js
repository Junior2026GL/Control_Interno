...
exports.create = (req, res) => {
  ...
  if (montoNum > MONTO_MAX) return res.status(400).json({ message: 'El monto excede el l�mite permitido.' });
  if (!bancoReceptor?.trim()) return res.status(400).json({ message: 'El banco receptor es requerido.' });
  if (!numeroCuenta?.trim()) return res.status(400).json({ message: 'El n�mero de cuenta es requerido.' });
  if (!fechaMes || !MESES_VALIDOS.includes(fechaMes)) return res.status(400).json({ message: 'El mes no es v�lido.' });
  const dia = parseInt(fechaDia, 10);
  if (!dia || dia < 1 || dia > 31) return res.status(400).json({ message: 'El d�a no es v�lido (1-31).' });
  const anio = parseInt(fechaAnio, 10);
  if (!anio || anio < 2000 || anio > 2100) return res.status(400).json({ message: 'El a�o no es v�lido.' });
