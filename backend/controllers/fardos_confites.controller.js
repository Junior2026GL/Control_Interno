...
exports.getAll = (req, res) => {
  db.query(
    `SELECT efc.*, u.nombre AS registrado_por
     FROM entrega_fardos_confites efc
     LEFT JOIN usuarios u ON u.id = efc.usuario_id
     ORDER BY efc.fecha_entrega DESC, efc.id DESC
     LIMIT 3000`,
    (err, results) => {
      if (err) {
        console.error('[entrega_fardos_confites] getAll:', err);
...
exports.create = (req, res) => {
  if (!ROLES_ADMIN.includes(req.user.rol))
    return res.status(403).json({ message: 'No tiene permisos para registrar entregas.' });
  ...
  if (!persona_retiro || persona_retiro.length < 2 || persona_retiro.length > 200)
    return res.status(400).json({ message: 'La persona que recogió los fardos es requerida (máx. 200 caracteres).' });
  ...
  db.query(
    `INSERT INTO entrega_fardos_confites
    ...
    (err, result) => {
      if (err) {
        console.error('[entrega_fardos_confites] create:', err);
        return res.status(500).json({ message: 'Error interno del servidor.' });
      }
      logEvent({
        usuario_id: req.user.id, usuario_nombre: req.user.nombre || null,
        accion: 'CREAR', modulo: 'fardos-confites',
        detalle: `Registró entrega de fardos — ${diputado_nombre}, ${departamento} (${cantNum} fardos)`,
        ip: getClientIP(req), metodo: req.method, ruta: req.originalUrl, resultado: 'EXITO',
      });
      res.status(201).json({ id: result.insertId, message: 'Entrega registrada correctamente.' });
    }
  );
};
...
exports.update = (req, res) => {
  if (!ROLES_ADMIN.includes(req.user.rol))
    return res.status(403).json({ message: 'No tiene permisos para editar entregas.' });
  ...
  if (!persona_retiro || persona_retiro.length < 2 || persona_retiro.length > 200)
    return res.status(400).json({ message: 'La persona que recogió los fardos es requerida.' });
  ...
  db.query(
    `UPDATE entrega_fardos_confites
    ...
    (err, result) => {
      if (err) {
        console.error('[entrega_fardos_confites] update:', err);
        return res.status(500).json({ message: 'Error interno del servidor.' });
      }
      ...
      logEvent({
        usuario_id: req.user.id, usuario_nombre: req.user.nombre || null,
        accion: 'ACTUALIZAR', modulo: 'fardos-confites',
        detalle: `Actualizó entrega de fardos ID #${id} — ${diputado_nombre}, ${departamento}`,
        ip: getClientIP(req), metodo: req.method, ruta: req.originalUrl, resultado: 'EXITO',
      });
...
exports.remove = (req, res) => {
  if (!ROLES_ADMIN.includes(req.user.rol))
    return res.status(403).json({ message: 'No tiene permisos para eliminar entregas.' });

  const id = parseInt(req.params.id, 10);
  if (!id || id <= 0) return res.status(400).json({ message: 'ID inválido.' });

  db.query('DELETE FROM entrega_fardos_confites WHERE id = ?', [id], (err, result) => {
    if (err) {
      console.error('[entrega_fardos_confites] delete:', err);
      return res.status(500).json({ message: 'Error interno del servidor.' });
    }
    if (result.affectedRows === 0)
      return res.status(404).json({ message: 'Registro no encontrado.' });
    logEvent({
      usuario_id: req.user.id, usuario_nombre: req.user.nombre || null,
      accion: 'ELIMINAR', modulo: 'fardos-confites',
      detalle: `Eliminó entrega de fardos ID #${id}`,
      ip: getClientIP(req), metodo: req.method, ruta: req.originalUrl, resultado: 'EXITO',
    });
